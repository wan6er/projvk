...
int main(int argv, char* args[])
{
    ...
    __cvk::get_device_memory_properties(physical_device, memory_properties);
    ...
    VkQueue queue;
    __cvk::get_device_queue(device, device.get_queue_family_index(VK_QUEUE_GRAPHICS_BIT), queue);

    VkSurfaceKHR surface;
    result = __cvk::create_win32_surface(instance, win.instance(), win, surface);
    ...
    __cvk::get_all_present_modes(physical_device, surface, present_modes);

    VkSwapchainKHR swapchain;
    result = __cvk::create_swapchain(physical_device, device, surface, present_modes[0], swapchain);
    ...
    VkMemoryRequirements depth_image_requirement;
    __cvk::get_image_memory_requirement(device, depth_image, depth_image_requirement);
    VkDeviceMemory memory;
    result = __cvk::create_memory(memory_properties, device, depth_image_requirement.size, depth_image_requirement.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, memory);
